 ...
 // uploadBaseMarket(bannerImageURL, req.body.banner);
  ...
  if (req.body.pages.length > 0){
    for (pIdx in req.body.pages){
      const foundPage = await informationPage.findOne({_id:req.body.pages[pIdx]});
      var obj = {
  ...
  const foundInfos = await information.findOne({_id:req.query.id})
    ...
    for (routeIdx in routeKeys){
      if (routeKeys[routeIdx].includes('image')){
        foundInfos._doc[routeKeys[routeIdx]] = `${process.env.S3_BASE}${foundInfos._doc[routeKeys[routeIdx]]}`
      };
    };
    ...
    for (bonusIdx in bonusCards){
      ...
      for (elementIdx in bonusDoc.elements){
        bonusDoc.elements[elementIdx].source.icon = `${process.env.S3_BASE}${bonusDoc.elements[elementIdx].source.icon}`
        console.log(bonusDoc.elements[elementIdx].source.icon)
      };
      for (fileIdx in bonusDoc.files){
        foundInfos._doc.bonus_cards[bonusIdx].id.files[fileIdx].url = `${process.env.S3_BASE}${bonusDoc.files[fileIdx].url}`;
        foundInfos._doc.bonus_cards[bonusIdx].id.files[fileIdx].icon = `${process.env.S3_BASE}${bonusDoc.files[fileIdx].icon}`;
      };
